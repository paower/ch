...
class UserComplaController extends AdminController
{
    ...
    public function index()
    {
        ...
        $where['d.status'] = 6;
        $list = $user_compla->join('ysk_deal d ON c.deal_id=d.id')->where($where)->limit($Page->firstRow.','.$Page->listRows)->order('c.status asc,c.id desc')->field('c.id,c.uid,d.img')->select();
        foreach($list as $k => $v){
    ...
    public function setYesVerify(){
        ...
        $user_arr['total_lingshi'] = array('exp','total_lingshi + '.$deal['num']);
        ...
        // 订单修改为已取消
        $res3 = M('deal')->where(array('id'=>$deal['id']))->setField(['status'=>5]);
        if($res && $res2 &&$res3){
            M()->commit();
            $this->success('投诉成功');
        }else{
