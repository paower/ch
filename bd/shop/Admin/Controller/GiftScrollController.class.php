...
class GiftScrollController extends AdminController
{
    ...
    public function add(){
        if(IS_POST){
            ...
            //矿机信息
            $scroll_info = M('scroll')->where("id = $scroll_id")->find();

            //赠送矿机
            $uscroll['uid'] = $is_mobile['userid'];
            $uscroll['scroll_id'] = $scroll_id;
            $uscroll['overtime'] = time() + $scroll_info['max_day'] * 86400;
            $uscroll['status'] = 0;
            $uscroll['max_day'] = $scroll_info['max_day'];
            $uscroll['sell_num'] = $scroll_info['sell_num'];
            $uscroll['single_num'] = $scroll_info['single_num'];
            ...
            if($res){
                M()->commit();
                $this->success('赠送矿机成功',U('GiftScroll/index'));
            }else{
                M()->rollback();
                $this->error('赠送矿机失败');
            }
