...
class LoginController extends Controller
{
    ...
    public function regiscode(){
        ...
        $userinfo = M('user')->where(array('account'=>$mobile))->find();
        if($userinfo){
            ajaxReturn('该账号已被使用');
        }
        session('registerphone',$mobile);
        session('registerusername',$username);
        
        ajaxReturn('ok',1,'register_2');
        
    }

    public function register_2(){
        $mp = session('registerphone');
        $up = session('registerusername');
        $fromurl="http://beid.com/Login/register.html"; //定义一个来路网址
    ...
    public function smsLogin(){
        if(IS_POST){
            $account = I('account');
            $verCode = I('verCode');
            $code = redis()->get("phone_sms:{$account}");
            if($code == $verCode){
                $user_object = D('Home/User');
