...
//        M('user')->where('userid='.$uid)->save(['vip_grade'=>$user_level_is]);
   ...
   public function Personal()
    {
        $uid = session('userid');
        $uinfo = M('user')->where(array('userid' => $uid))->field('username,is_dailishang,userid,img_head,use_grade,vip_grade,total_lingshi,total_active,mobile')->find();
		...
		$this->assign('is_turntable_time',$is_turntable_time);
    ...
    // public function pays(){
        ...
        // if($is_real_name==1 && $is_pay==0){
            // $user_pay = M('user_pay')->where(array('uid'=>$userid))->order('id desc')->find();
            // $paydata=array();
            // if(!isset($user_pay)){
...
//        $u_ID = M('user')->where(array('userid'=>$userid))->getField('mobile');
    ...
    public function Teamdets()
    {
        ...
        $muinfo = M('user')->where($where)->order('userid desc')->field('username,vip_grade,userid,mobile,total_active,reg_date,img_head')->select();
		foreach($muinfo as $k => $v){
            $team_list = $this->getTeamChild($v['userid'],$data);
            $team_active = 0;
            foreach($team_list as $val){
    ...
    public function phone_recharge(){
        ...
        if($total_lingshi < $price_num+5){
            ajaxReturn('您的CC币不足,账户需保留5个',0);
        }
        if($mobile_number == ''){
            ajaxReturn('请输入手机号',0);
        }
