        ...
        if(IS_AJAX){
            ...
            if($size){
                ...
                if($is_default){
                    ...
                    M('ubanks')->where(array('user_id'=>$uid,'is_default'=>1))->setField('is_default',0);
                }
                ...
                if($res){
                    ajaxReturn('提交支付宝号成功',1,'/Growth/Cardinfos');
                }else{
...
//卖入中心
    ...
    public function Cardinfos(){
        $uid = session('userid');
        $morecars = M('ubanks as u')->where(array('u.user_id'=>$uid))->order('u.id desc')->field('u.alipay_name,u.id,u.alipay_number,u.user_id,u.img,u.is_default')->select();
        if(IS_AJAX){
            $cardid = I('bangid');
            //是否是自己绑定的银行卡
            $isuid = M('ubanks')->where(array('id'=>$cardid))->getField('user_id');
            if($isuid != $uid){
                ajaxReturn('该支付宝暂不属于您~',0);
            }
            $res = M('ubanks')->where(array('id'=>$cardid))->delete();
            if($res){
                ajaxReturn('该支付宝删除成功',1,'/User/Personal');
            }
        }
        $this->assign('morecars',$morecars);
        $this->display();
    ...
    public function default_bank(){
        ...
        if($ubanks !== false){
            ajaxReturn('设置默认支付宝成功',1,'/User/Personal');
        }else{
            ajaxReturn('设置默认支付宝失败',0);
        }
    }
}