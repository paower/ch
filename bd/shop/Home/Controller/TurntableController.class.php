...
// NYT转入
    public function WBgetin(){
        ...
        $wbao_ss = M('wbao_detail')->add($wbaoss);
            ...
            ajaxReturn('CC币币转入成功',1,"Wbaobei");
        }else{
            ajaxReturn('CC币币转入失败',0);
        }
    }
...
//    Ypay转入
    public function Wegetin(){
        ...
        if($res_tran){
            ajaxReturn('CC币币转出成功',1,"index");
        }else{
            ajaxReturn('CC币币转出失败',0);
        }
    }
    ...
    public function transaction(){
        ...
        foreach ($list as $kname => $vname) {
            $length = mb_strlen($vname['username'],'UTF8');
            if($length>0){
                $first = mb_substr($vname['username'],0,1,'utf-8') . '*';
                $last  = '';
                if($length >= 3) {
                   $last  = mb_substr($vname['username'], -1, 1,'utf-8');
                }
            }
            $list[$kname]['username'] = $first . $last;
        }
...
//防重复提交
    ...
    public function quxiao(){
        ...
        if($deal['type'] == 1){
            //返还CC币,出售
			$vip_grade = M('user')->where(array('userid'=>$deal['sell_id']))->getField('vip_grade');
			$tran_fee = M('user_level')->where(array('vip_grade'=>$vip_grade))->getField('tran_fee');
            $lingshi = $deal['num'] + $deal['num'] * $tran_fee / 100;
...
//取消订单
 public function quxiao_order(){
    ...
    public function T_Salesell(){
        ...
        if(!$num|!$tprice)ajaxReturn('CC币的数量不能为空~',0);
...
//        }
  ...
  public function Crowds_goumai(){
    ...
    public function sell_this(){
		...
		$vip_grade = M('user')->where(array('userid'=>$uid))->getField('vip_grade');
		$tran_fee = M('user_level')->where(array('vip_grade'=>$vip_grade))->getField('tran_fee');
		$num = $deal['num'] + $deal['num'] * $tran_fee;
        //冻结我的CC币
        // $payout1['djie_num'] = array('exp', 'djie_num + ' . $deal['tprice']);
        $payout1['total_lingshi'] = array('exp', 'total_lingshi - ' . $num);
        $res_pay3 = M('user')->where(array('userid'=>$uid))->save($payout1);
    ...
    public function recharge(){
        ...
        if($type == 1){
            ...
            $vip_grade = M('user')->where(array('userid'=>$uid))->getField('vip_grade');
            $tran_fee = M('user_level')->where(array('vip_grade'=>$vip_grade))->getField('tran_fee');
            $tran_price = $deal['num'] * $tran_fee / 100;

            $deal['tprice'] = $deal['tprice'];
        ...
        $recharge_token = getCode();
        session('recharge_token',$recharge_token);
        $this->assign('recharge_token',$recharge_token);
        $this->assign('uid',$uid);
        $this->display();
    }
    ...
    public function Conpay(){
        ...
        if(IS_AJAX){
            ...
            $deal = M('deal')->where(array('id'=>$trid))->field('expiration_time,type,pay_id,sell_id')->find();
            if(time() >= $deal['expiration_time']){
            ...
            if($size){
                $res = M('deal')->where(array('id'=>$trid))->setField(array('img'=>$pic_path,'status'=>2,'expiration_time'=>time()+2*3600));
                if($res){
                    $mobile = M('user')->where(array('userid'=>$deal['sell_id']))->getField('mobile');
                    $tpl_id = 167309;
                    
                    $this->sendCode($mobile,$tpl_id);
					
                    ajaxReturn('打款提交成功',1,U('Turntable/Transreocrds'));
                }else{
                    ajaxReturn('打款提交失败',0);
                }
            }
    ...
    public function confirm_receipt(){
        $id = (int)I('id');
        $deal = M('deal')->where(array('id'=>$id,'status'=>2))->find();
        if(!$deal){
            $this->error('该交易不存在，请刷新重试');
        }
        if(IS_POST){
            $recharge_token = trim(I('recharge_token'));
            if($recharge_token&&$recharge_token != session('recharge_token')){
                $ajax_arr['code'] = 0;
                $ajax_arr['msg'] = '请勿重复提交';
                exit(json_encode($ajax_arr));
            }
            session('recharge_token',getCode());

            $id = (int)I('id');
            $deal = M('deal')->where(array('id'=>$id))->find();
             // if(time() >= $deal['expiration_time']){
                 // $this->success('该交易已过期,自动收款');
             // }

            $res = M('deal')->where(array('id'=>$deal['id']))->setField(['status'=>3,'end_time'=>time()]);
            //出售用户
            // $tran_fee_lingshi = ($deal['tprice']-$deal['fee_num'])/$deal['dprice'];
            $lingshi = $deal['num']; //CC币数量
            // M('user')->where(array('userid'=>$deal['sell_id']))->setDec('djie_num',$lingshi);
            // 购买用户
            M('user')->where(array('userid'=>$deal['pay_id']))->setInc('total_lingshi',$lingshi);

            //添加记录
            $total_lingshi = M("user")->where(array('userid'=>$deal['pay_id']))->getField('total_lingshi');
            $tm['pay_id'] = 0;
            $tm['get_id'] = $deal['pay_id'];
            $tm['get_nums'] = $lingshi;
            $tm['get_time'] = time();
            $tm['get_type'] = 32;
            $tm['now_nums'] = $total_lingshi;
            ...
            //等级2及以上会员收取手续费
            // $users = M('user')->where(array('vip_grade'=>array('egt',2)))->field('service_charge,userid')->select();
            // foreach($users as $k => $v){
                // $num = $deal['fee_num'] * 20 / 100;
                // $num = $num*$v['service_charge']/100;
                // M('user')->where(array('userid'=>$v['userid']))->setInc('total_lingshi',$num);
            // }

            // $user_star = M('user_star')->select();
            // foreach($user_star as $k => $v){
            //     $star_user = M('user')->where(array('user_star_id'=>$v['id']))->field('userid')->select();
            //     if($star_user){
            //         //计算所获取得手续费
            //         $num = $deal['fee_num'] * $v['get_fee'] / 100;
            //         $count = count($star_user);
            //         $num = $num / $count;
            //         foreach($star_user as $key => $val){
            //             M('user')->where(array('userid'=>$val['userid']))->setInc('total_lingshi',$num);
            //         }
            //     }
            // }
            ...
            if($res !== false){
                ajaxReturn('提交成功',1,U('Turntable/Transreocrds'));
            }else{
                ajaxReturn('提交失败');
            }
        }
    ...
    public function first(){
        ...
        if($orders['type'] == 2){
            ...
            $tran_fee = M('user_level')->where(array('vip_grade'=>$vip_grade))->getField('tran_fee');
            $tran_price = $orders['num'] * $tran_fee / 100;

            $orders['tprice'] = $orders['tprice'];
    ...
    public function confirm_represen(){
        ...
        $deal = M('deal')->where(array('sell_id'=>$uid,'id'=>$id))->count();
        if(!$deal){
            ajaxReturn('该订单不存在',0);
        }
        ...
        if($res){
            ajaxReturn('申诉成功，等待后台审阅',1,U('Turntable/Transreocrds'));
        }else{
	...
	public function confirm_cancel(){
		$uid = session('userid');
		$id = (int)I('id');
		$deal = M('deal')->where(array('id'=>$id,'sell_id'=>$uid))->find();
		if(!$deal){
		...
		if($deal['type'] == 1){
			...
			$num = $deal['num'] + $deal['fee_num'];
			$user_arr['total_lingshi'] = array('exp','total_lingshi + '.$num);
			$res = M('user')->where(array('userid'=>$deal['sell_id']))->save($user_arr);
		...
		$res = M('deal')->where(array('id'=>$id))->save($arr);
		if($res){
			ajaxReturn('取消订单成功',1,'/Turntable/Transaction');
		}else{
			ajaxReturn('取消订单失败',0);
		}
	}
