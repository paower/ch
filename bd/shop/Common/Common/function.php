...
//    dump($openid);die();
    return M('user')->where(array('openid'=>$openid))->getField('userid');
}
...
function build_username()
{
    ...
    //检测是否存在
    $db = M('user', 'ysk_');
    $count = $db->where(array('username' => $uid))->count(1);
    ($count > 0) && $uid = build_username();
    return $uid;
}

function randstr_wallet(){
    $strs="1234567890qwertyuiopasdfghjklzxcvbnm";
    $name=substr(str_shuffle($strs),mt_rand(0,strlen($strs)-11),38);
    return sha1($name);
}
function build_order_no()
{
    ...
    //检测是否存在
    $db = M('deal', 'ysk_');
    $count = $db->where(array('deal_sn' => $no))->count(1);
