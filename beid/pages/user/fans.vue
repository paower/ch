				...
				<block v-for="(item,index) in userlist" :key="index">
					<view class="uni-comment-list">
						...
						<view class="uni-comment-body">
							...
							<view class="uni-comment-content">关注了你</view>
							<view class="uni-comment-date">
								<view class="uni-comment-replay-btn1" v-if="item.is_my_follow==false" @click="addfollow(item.uid)">关注</view>
								<view class="uni-comment-replay-btn" v-if="item.is_my_follow==true" @click="delFollow(item.uid)">相互关注</view>
							</view>
						</view>
					</view>
				</block>
...
<script>
    ...
    export default {
		...
		methods:{
			...
			MyFansList(){
				api.get({
					url:'Api/FollowApi/GetMyFansList',
